...
def build_gauss_kernel(size=5, sigma=1.0, n_channels=1, cuda=False):#构建高斯内核
    if size % 2 != 1:
        raise ValueError("kernel size must be uneven")#内核大小必须不均匀
    ...
    # repeat same kernel across depth dimension在深度维度上重复相同的内核
    kernel = np.tile(kernel, (n_channels, 1, 1))
    # conv weight should be (out_channels, groups/in_channels, h, w),转换权重应该是 (out_channels, groups/in_channels, h, w),
     # 因为我们有深度可分离的卷积，所以我们希望组的维度为 1
...
def conv_gauss(img, kernel):
    # conv img with a gaussian kernel that has been built with build_gauss_kernel使用 build_gauss_kernel 构建的高斯内核 conv img
    n_channels, _, kw, kh = kernel.shape
    img = F.pad(img, (kw // 2, kh // 2, kw // 2, kh // 2), mode='replicate')
    return F.conv2d(img, kernel, groups=n_channels)


def laplacian_pyramid(img, kernel, max_levels=5):#拉普拉斯金字塔
    current = img
    pyr = []
...
def define_BoundaryVAE(input_nc, output_nc, ngf, ndf, latent_variable_size, gpu_ids=[]):#定义边界
    ##### BoundaryVAEv20
...
class MultiDilationResnetBlock_attention(nn.Module):
    def __init__(self, input_nc_each, kernel_size=3, stride=1, padding=1, dilation=1, groups=1, bias=True,
        ...
        ### hard code, 4 dilation levels硬编码，4个膨胀级别
