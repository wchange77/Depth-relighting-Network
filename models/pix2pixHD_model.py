...
class Pix2PixHDModel(BaseModel):
    ...
    def init_loss_filter(self, use_gan_feat_loss, use_vgg_loss):
        ...
        def loss_filter(g_l1, g_gan, g_gan_feat, g_vgg, d_real, d_fake):
            return [l for (l, f) in zip((g_l1, g_gan, g_gan_feat, g_vgg, d_real, d_fake), flags) if f]
    ...
    def initialize(self, opt):
        ...
        self.netG = networks_DMSN2.define_G(netG_input_nc, opt.output_nc, opt.ngf, opt.netG,
        ...
        if self.isTrain:
            ...
            if not opt.no_instance:
                netD_input_nc += 1
            self.netD = networks_DMSN2.define_D(netD_input_nc, opt.ndf, opt.n_layers_D, opt.norm, use_sigmoid,
                                               opt.num_D, not opt.no_ganFeat_loss, gpu_ids=self.gpu_ids)

        ### Encoder network
        if self.gen_features:
            self.netE = networks_DMSN2.define_G(opt.output_nc, opt.feat_num, opt.nef, 'encoder',
        ...
        if self.isTrain:
            ...
            self.criterionGAN = networks_DMSN2.GANLoss(use_lsgan=not opt.no_lsgan, tensor=self.Tensor)
            #self.criterionFeat = torch.nn.L1Loss()
            self.criterionFeat = L1_Charbonnier_loss()
            #self.criterionL1 = torch.nn.L1Loss()
            self.criterionL1 = L1_Charbonnier_loss()
            self.criterionSSIM = pytorch_ssim.SSIM()
            self.criterionMSE = torch.nn.MSELoss()
            # self.criterionLPIPS = LPIPS(
            #     net_type='vgg',  # choose a network type from ['alex', 'squeeze', 'vgg']
            #     version='0.1'  # Currently, v0.1 is supported
            # ).cuda()
            #self.criterionPSNR = kornia.losses.PSNRLoss()
            self.criterionLAP = LapLoss()
            if not opt.no_vgg_loss:
                self.criterionVGG = networks_DMSN2.VGGLoss(self.gpu_ids)

            # Names so we can breakout loss
            self.loss_names = self.loss_filter("G_rec", 'G_GAN', 'G_GAN_Feat', 'G_VGG', 'D_real', 'D_fake')
        ...
        #self.netG.load_newhdrmodel()  # add by Liwen
    ...
    def forward(self, img_in, img_tar,img_gray,infer):
        # Encode Inputs
        # input_label, inst_map, real_image, feat_map = self.encode_input(label, inst, image, feat)
        img_in = img_in.cuda()
        real_image = img_tar.cuda()
        ...
        fake_image,fake_image_gray = self.netG.forward(img_in,img_gray)
        ...
        if not self.opt.no_vgg_loss:
            # loss_G_VGG = self.criterionVGG(fake_image, real_image) * self.opt.lambda_feat*0.05
            #loss_G_VGG = self.criterionLPIPS(fake_image, real_image) * self.opt.lambda_feat * 0.05#\
