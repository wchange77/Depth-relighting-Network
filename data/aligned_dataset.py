...
class TraindataFromTrack3(data.Dataset):
    ...
    def __init__(self):
        ...
        self.image_folder = "/work_dir/wangwei/tr/paper/MCN1/dataset/train/train"
    ...
    def is_image_GTfile(self, filename):
        ...
        #return any(filename.endswith(extension) for extension in ["_HDR.png"])
...
###################################################################################################################
        if int(InFileName[5:8]) >= 300:
            isUseFlipAug = False
            InTargetName = InFileName[:8] + "_4500_E.png"
            print('val#############################################',InTargetName )
            #InTargetName = InFileName[:8] + "_HDR.png"
        else:
            if bool(random.getrandbits(1)):
                isUseFlipAug = False
                InTargetName = InFileName[:8] + "_4500_E.png"
                print('trainbase#############################################',InTargetName )
                #InTargetName = InFileName[:8] + "_HDR.png"

            else:
                isUseFlipAug = True
                InTargetName = InFileName[:8] + "_4500_W.png"
                print('trainFilpAUG#############################################',InTargetName )
                #InTargetName = InFileName[:8] + "_HDR.png"
###################################################################################################################
        ...
        if bool(random.getrandbits(1)):
            isUseFlipAug = False
            print('falseInFileName[:8]##############################################',InFileName[5:8] )
            InTargetName = InFileName[:8] + "_4500_E.png"
            #InTargetName = InFileName[:8] + "_HDR.png"

        else:
            isUseFlipAug = True
            print('TrueInFileName[:8]##############################################',InFileName[5:8] )
            InTargetName = InFileName[:8] + "_4500_W.png"
            #InTargetName = InFileName[:8] + "_HDR.png"
        '''
        ...
        #print(Image.open(InputFile).size)
...
############################################################修改#####################################################
        A_tensor = self.transform(img_input.convert('RGB'))
        B_tensor = self.transform(img_target.convert('RGB'))
        ...
        r,g,b = A_tensor[0]+1, A_tensor[1]+1, A_tensor[2]+1
...
############################################################修改#####################################################
        input_dict = {'input': A_tensor, 'target': B_tensor, 'A_gray': A_gray}
